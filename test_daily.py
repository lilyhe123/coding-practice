...
def getDecodingWays(msg):
  dp0,dp1,dp2 = 1,0,0
  # '111' 1,2,3
  if int(msg[0]) > 0: dp1 = 1

  for i in range(1, len(msg)):
    if int(msg[i]) > 0: dp2 += dp1
    if int(msg[i-1:i+1]) in range(10, 27): dp2 += dp0
    dp0 = dp1
    dp1 = dp2
    dp2 = 0
  return dp1
...
def getUnivalTreeNum(node):
  def dfs(node):
    ...
    # leaf node
    if not node.left and not node.right: return [True, 1]
...
def test_8():
  root = TreeNode(0, left= TreeNode(1), right= TreeNode(0, left=TreeNode(1, left=TreeNode(1),
...
let's assume already pick an element randomly in [0, i-1]. Now ith element comes in, we need to pick up the ith element at 1/(i+1) chance. so if random(1, i+1) == i, change to picked elment to the ith. otherwise, the picked element remains no change.

reservoir sampling
probability to choose 1th element when process 5 elements:
1 * 1/2 * 2/3 * 3/4 * 4/5 = 1/5
probability to choose 2th element when process 5 elements:
1/2 * 2/3 * 3/4 * 4/5 = 1/5
probability to choose 2th element when process 5 elements:
1/3 * 3/4 * 4/5 = 1/5
'''
  ...
  def compareAndSync(self, localNodes, remoteNodes):
    ...
    if len(localNodes) > len(remoteNodes):
      if not localNodes[-1].isLeaf():
        ...
        print("find local last block is leftover, remove it?", localNodes[-1].hash)
...
def wordExists(matrix, word):
  ...
  def dfs(r, c, index):
    ...
    if index == len(word)-1:
      return True
    return (r+1 < ROWS and dfs(r+1, c, index+1)) or (c+1 < COLS and dfs(r, c+1, index+1))
...
def spinMatrix(matrix):
  ...
  while len(steps) < TOTAL:
    for dr,dc in directions:
      while 0<=row+dr<N and 0<=col+dc<M and matrix[row+dr][col+dc] > 0:
        row += dr
        col += dc
        steps.append(matrix[row][col])
        matrix[row][col] = 0
